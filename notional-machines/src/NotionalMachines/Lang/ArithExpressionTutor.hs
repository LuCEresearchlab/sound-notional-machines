...
pattern NodeIfThenElse i <- MkNode i [Hole _, Hole _, Hole _] where
        NodeIfThenElse i =  MkNode i [holeP,  holeP,  holeP]
...
arithToET = langToET go
  where go :: Term -> State Int ExpTreeDiagram
        go = \case
          ...
          IfThenElse t1 t2 t3 -> newDiaBranch NodeIfThenElse go [t1, t2, t3]
...
etToArith = etToLang go
  where
    ...
    go = \case
      ...
      DiaBranch (NodeIfThenElse i) ts @ [_, _, _] -> let [n1, n2, n3] = fmap go ts
                                                     in checkCycle i (liftM3 IfThenElse n1 n2 n3)
