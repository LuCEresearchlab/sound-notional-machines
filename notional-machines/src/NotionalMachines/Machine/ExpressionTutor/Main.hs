...
pattern DiaBranch :: Node -> [ExpTutorDiagram] -> ExpTutorDiagram
...
diaBranch :: ExpTutorDiagram -> Maybe (Node, [ExpTutorDiagram])
...
newDiaLeaf :: (Int -> Node) -> State Int ExpTutorDiagram
...
newDiaBranch :: (Int -> Node) -- ^ constructor to create branching node.
             -> (t -> State Int ExpTutorDiagram) -- ^ function to recursively build next nodes.
             -> [t] -- ^ the terms used to construct the next nodes.
             -> State Int ExpTutorDiagram
newDiaBranch c f xs = do ys <- mapM f xs
                         incUid $ \uid -> DiaBranch (c uid) ys

incUid :: (Int -> ExpTutorDiagram) -> State Int ExpTutorDiagram
...
-- otherwise).
checkCycle :: ExpTutorDiagram -> StateT (Set Int) Maybe b -> StateT (Set Int) Maybe b
